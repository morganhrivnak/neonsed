...
mod_posteriors_summary_2 <- mod_posteriors_summary %>% 
  ggplot(aes(x = percent_s, y = .epred, color = year)) +
  geom_line() +
...
#unstandardizing the model (aka multiplying by max size to back transform it)

mod_posteriors_unstd <- mod_posteriors %>%
  mutate(
    percent = (percent_s * sigma_percent) + mu_percent,  # Convert percent_s back
    size = .epred * max_size  # Convert predicted size_s back
  ) %>% 
  median_qi(.epred)

 
ggplot(mod_posteriors_unstd, aes(x = percent_s, y = .epred, color = year)) +
  geom_line() +
  facet_wrap(~site, scales = "free_y") +
  geom_ribbon(aes(ymin = .lower, ymax = .upper, fill = year), alpha = 0.2) +
  geom_point(data = mod_posteriors_unstd, aes(y = .epred))
